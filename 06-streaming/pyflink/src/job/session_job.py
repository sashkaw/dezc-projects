...
def create_events_source_kafka(t_env):
    ...
    source_ddl = f"""
        CREATE TABLE {table_name} (
            lpep_pickup_datetime CHARACTER(19),
            lpep_dropoff_datetime CHARACTER(19),
            ...
            WATERMARK for event_watermark as event_watermark - INTERVAL '5' SECONDS
        ) WITH (
            'connector' = 'kafka',
...
def log_aggregation():
    ...
    try:
        ...
        # TODO: Fix this -> KafkaConsumer works to read messages
        # but job is not updating the PostgreSQL table
        t_env.execute_sql(f"""
            ...
            --FROM SESSION(TABLE {source_table} PARTITION BY PULocationID, DOLocationID, DESCRIPTOR(event_watermark), INTERVAL '5' MINUTES)
